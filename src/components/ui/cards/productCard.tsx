    ...
    <div className="group">
      <div className="relative">
        <div className="absolute z-10 w-full h-full flex justify-center items-center opacity-0 group-hover:opacity-100 transition-all duration-500">
          <button
            onClick={() => dispatch(addToCart({ id, quantity: 1, product_img, product_name, price }))}
            className={cn(
      ...
      <div className="mt-[21px] flex justify-between">
        <div>
          <Link
            href={`/product-single`}
            className={cn(`text-2xl font-semibold text-primary-foreground leading-160 hover-underline ${text_muted}`)}
          >
            {' '}
