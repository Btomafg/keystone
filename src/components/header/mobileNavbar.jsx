        ...
        <div className="container-fluid ">
          <div className="flex items-center justify-between py-4">
            ...
            <div>
              <nav
                style={{ zIndex: 1000 }}
                className={`max-h-screen min-h-screen bg-[#F9FFFC] absolute transition-all duration-500 ${
                  isMenuActive ? 'right-0' : 'hidden md:block md:-right-[150%]'
                } top-0 py-4 px-4`}
              >
                      ...
                      <li key={id} onClick={(e) => handleDropDown(e, id, isDropdown, isMegaMenu)}>
                        <Link
                          ...
                          className={`nav-link text-xl font-medium px-4 py-4 flex items-center gap-2 transition-all duration-500 text-primary-foreground ${
                            dropDownActive === id ? 'bg-primary text-secondary-foreground' : ''
                          }`}
                        >
                          {name}
                          {(isDropdown || isMegaMenu) && (
                            <span
                              className={` transition-all duration-500 rotate-180 ${
                                dropDownActive === id ? 'rotate-0 text-secondary-foreground' : ''
                              }`}
                            >
                        ...
                        <ul
                          className={`transition-all duration-500 max-h-0 overflow-hidden px-4 pb-2 ${
                            dropDownActive === id ? 'max-h-[500px] bg-primary text-secondary-foreground ' : ''
                          }`}
                        >
                          {isDropdown &&
                        ...
                        {isMegaMenu && (
                          <ul
                            className={`transition-all duration-500 max-h-0 overflow-hidden px-4 pb-2 ${
                              dropDownActive === id ? 'max-h-[700px] bg-primary text-secondary-foreground ' : ''
                            }`}
                          >
