    ...
    <div className="space-y-4 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col ">
        ...
        <Table>
          ...
          <TableBody>
            {project?.rooms?.sort((a,b) => a.id - b.id)?.map((room: any) => (
      ...
      <div className="flex flex-col border-t border-t-muted-foreground pt-4 !mt-24 ">
        Estimated Project Cost: <span className="text-lg font-semibold ms-auto">{toUSD( project?.estimate )|| 'Create Cabinets For Estimate'}</span>
      </div>
    ...
    <TableRow className={`cursor-pointer hover:bg-blue-50  ${isOpen && 'border-b-0'}`} onClick={() => toggleWalls(room.id)}>
      ...
      <TableCell>
        {editRoomOpen ? (
          ...
          room?.height ? `${room?.height} ft` : 'N/A'
        )}
      </TableCell>
      ...
      <TableCell>
        {editRoomOpen ? (
          <div className="flex gap-2">
            ...
            <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); setEditRoomOpen(false); }}>Cancel</Button>
          </div>
        ) : (
          <DropdownMenu>
...
const WallRow = ({
  ...
  wall: any;
  room: any;
  wallImage: (roomId: number, wallNumber: number) => string;
}) => {
        ...
        <div className="flex items-center">
          ...
          <div className="ml-4 flex flex-row gap-2">
            <div  className="ml-4 flex flex-col gap-2">
              ...
              <Input
                value={editedWallName}
                onChange={(e) => setEditedWallName(e.target.value)}
              />
            </div>
            <div  className="ml-4 flex flex-col gap-2">
  ...
  return (
    <div className="flex flex-col mb-4 border-t border-t-muted-foreground pt-2 px-4">
        ...
        <div className="flex flex-row justify-between w-full items-center">
          ...
          <div className="mx-auto flex flex-col gap-1 w-fit">
            ...
            <div className="text-sm">
              {wall?.length ? `Length: ${wall.length}ft` : <div className='text-blue-400 flex hover:cursor-pointer' onClick={() => setEditWallOpen(true)}>Add Wall Length</div>}
            </div>
          </div>
          <div className="mx-auto flex flex-col gap-1 w-fit">
      ...
      {wall.cabinets && wall.cabinets.length > 0 && (
        <CabinetsList cabinets={wall.cabinets} />
      )}
    </div>
  );
};
...
const CabinetRow = ({ cabinet }: { cabinet: Cabinet }) => {
  ...
  return (
    <div className="flex-col items-center justify-between p-2 border rounded-md bg-muted/50 shadow-sm hover:bg-blue-50 hover:cursor-pointer mb-2" onClick={() => setRowOpen(!rowOpen)}>
      ...
      <div className="flex flex-row gap-2">
          ...
          <div className="mx-auto flex flex-col gap-1 w-fit">
            ...
            <div className="text-sm">
              {cabinet?.length ? `${cabinet?.length}ft` : ''}
            </div>
          </div>
          <div className="mx-auto flex flex-col gap-1 w-fit">
            <div className="font-semibold">Height</div>
            <div className="text-sm">
              {cabinet?.length ? `${cabinet?.height}ft` : ''}
            </div>
          </div>
          <div className="mx-auto flex flex-col gap-1 w-fit">
            <div className="font-semibold">Estimated Cost</div>
      ...
      <div className="mx-auto flex flex-col gap-1 w-fit"><ChevronUp className={`w-6 h-6 transition-transform  my-auto align-middle ${rowOpen && 'rotate-180'}`} />
      </div>
      </div>
