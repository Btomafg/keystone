...
export default function NewProjectCabinets({ project, onBack }: NewProjectCabinetsProps) {
  ...
  return (
    <div style={{ maxWidth: screenWidth * 0.9 }} className="space-y-6 max-w-md md:max-w-2xl mx-auto">
      {editingCabinetId && (
        <CabinetBuilderModal
          step={modalStep}
  ...
  return (
    <div className="flex justify-between items-center">
      <div className="flex items-center gap-2">
        {isEditing ? (
          <Input value={tempName} onChange={(e) => setTempName(e.target.value)} placeholder="Enter room name" className="w-full" />
        ) : (
      ...
      <Popover>
        ...
        <PopoverContent className="flex flex-col gap-2 p-4 bg-white text-black rounded-md shadow-md">
          ...
          <div className="flex flex-row justify-between gap-2">
            ...
            <Button size="xs" variant="destructive" onClick={() => deleteRoom(room.id)}>
              Yes
            </Button>
          </div>
        </PopoverContent>
