    ...
    <div className="absolute z-50 left-1/2 -translate-x-1/2 bg-primary flex max-w-[1320px] w-full transition-all duration-500 max-h-0 overflow-hidden  group-hover:max-h-[370px] ">
      {dropDownList.map(({ menus, id }) => {
        return (
          <ul
            key={id}
            className='w-full relative after:absolute after:top-0 after:-right-1 after:contents-[""] after:bg-primary_rgba last:after:bg-transparent after:w-[2px] after:h-full'
          >
                  ...
                  {img ? (
                    ...
                    <div className="px-6 pt-7.5 ">
                      <Link
                        href={path}
                        data-id={parentId}
                        className='dropdown-item text-secondary-foreground text-lg font-bold  inline-block relative after:transition-all after:duration-700 after:absolute after:left-0 after:bottom-0 after:contents-[""] after:w-0 after:h-[2px] after:bg-secondary after:hover:w-full '
                      >
                        {name}
                      </Link>
                      <span className="block text-secondary_rgba text-sm mt-3">{desc}</span>
                    </div>
                  )}
                </li>
              );
