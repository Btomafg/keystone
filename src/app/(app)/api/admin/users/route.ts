...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data, error } = await supabase.from('Users').update(body).eq('id', body.id).select('id').single();
    ...
    if (data) {
      return NextResponse.json({ success: true, message: 'User updated successfully', type: 'success' }, { status: 200 });
    }
  } catch (err: any) {
    console.error(err);
    return NextResponse.json({ success: false, message: 'Server error', type: 'error' }, { status: 500 });
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const { data, error } = await supabase.from('Users').delete().in('id', body.Users);
